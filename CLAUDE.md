...
| `docs/` | 架构事实与设计叙事，四级体裁见 [docs/index.md](docs/index.md) | 人和 agent 主动查 |
| `AGENTS.md` | 只该是 CLAUDE.md 的压缩派生，**不手工维护第二套说法** | 其他 agent（Codex/Cursor）读 |
| 用户 memory | 个别的行为校准、踩过的坑、临时偏好 | 跨会话自动带入，不进仓库 |

裁决顺序：先问「是不是每次会话都要生效」——否则不进 CLAUDE.md；再问「有没有明确的触发时机」——有则进 skill；剩下的按性质进 `docs/` 或 memory。

对 CLAUDE.md 里的机制约束做改动时，同次核对 `AGENTS.md` 是否需要跟着派生——它不在 `.gitignore` 里，是随公开仓库分发的第三份副本。
...
**产出顺序：先交流信息 → 再修正项目 → 最后才决定落盘到哪里。** 这和实现模式是**反的**（实现模式先定验收标准再动手），别让实现模式的惯性覆盖它。出了结论的研究进 `docs/research/<topic>.md`（允许没有结论，保留被推翻的假设与撤回的结论），或催生 `research` 标签的 issue。
...
**机制演进与收口**：对承载行为、状态或编排的改动，先说明现有权威路径、生产者、消费者与生命周期。语义相同则扩展既有路径；只有职责、生命周期或事实源确实不同才新增。每项关切只保留一个权威表示。存在先后依赖时由明确调用者编排，**不得依赖自动发现顺序、目录顺序或后台队列时序**。本次若明确替换某条路径，同次移除被它替代的旧入口、配置、文档、提示词与无调用脚手架——被替代路径即「直接相关」，但清理边界到此为止，不扩大到任务无关的历史代码。
