...
本文讲机制。**为什么是这个样子**见 [记忆的想法&构思](MEMORY-DESIGN.md)；架构总览见 [架构文档](ARCHITECTURE.md)，配置项见 [配置说明](CONFIGURATION.md)。
...
前面几节讲的是「记住」。但**记住从来不是我的目标**——我要的是一个能自己生长的 agent，记忆只是实现它所必需的底座（这条主线的来历见[记忆的想法&构思](MEMORY-DESIGN.md)）。
...
- **最根本的一条：类型集合本身还是写死的。** 特征词、维度、技能都会长，但「一共有哪几类记忆」还是我定的——`slices_pipeline` 里只有 `user_info` 与 `task` 两个硬编码分支。把这个也解开才算是真的自涌现，那是主线的下一段，见[想法&构思那一篇](MEMORY-DESIGN.md#什么是自涌现现在是什么程度潜力或者我对他未来的期望是什么)
...
上面写的是「现在是什么样」。**为什么是这个样子**——包括那个讨论最久却主动没有落地的图结构、Hook 系统其实源于一次切片卡顿、以及一个我自己测出来的反直觉结论——单独写在了 **[记忆的想法&构思](MEMORY-DESIGN.md)**。
