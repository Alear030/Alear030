...
- **Classification must fail conservatively**. The git subcommand tables do not try to be exhaustive: when several entries match, the highest severity wins, and when none matches the category falls to unknown rather than the command's own neutral. The dangerous-path check passes `read` and `neutral` straight through, which put "not recognized" and "confirmed read-only" in the same exempt bucket — `GIT_READ_ONLY` was once a dump of every git subcommand, so `git rm <system path>` skipped that layer entirely
- **Recursive-delete detection works on parsed tokens, not on raw command text**. As a regex it needs an anchor requiring `rm` at the start of a segment to avoid matching literals inside quotes, so anything that wraps `rm` one layer deep (`git rm -rf .`) escaped the hard block completely; after segmentation `base_cmd` and its flags are already available, and the check placed there does not depend on an anchor
- **Flag values go through the dangerous-path check just like positional arguments**. The path actually being touched often appears only after `-o`, `-C`, or `--prefix=`, so scanning positionals alone lets the real target through. The value types in `safe_flags` split three ways: `number`/`string` values are not scanned as paths (otherwise a commit message containing `/etc` would be blocked), `path` values are checked under the command's own category, and `outpath` values (`curl -o`, `wget -O`) are always checked as writes — those two commands are classified read, so their own category would pass them straight through, and an output target is the same thing as a redirect target
