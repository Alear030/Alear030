...
- **分类的失败方向必须保守**。git 子命令表不追求穷举：命中多个集合取最高危险档，一个都没命中落 unknown 而不是命令自身的 neutral。危险路径检查对 `read` 与 `neutral` 都直接放行，等于把「没认出来」和「确认只读」放进同一个免检桶——`GIT_READ_ONLY` 曾是 git 全量子命令转储，`git rm <系统路径>` 因此整层跳过
- **递归删除的判定落在已解析的 token 上，不在原始命令文本上**。写成正则时锚点必须要求 `rm` 出现在段首才不误伤引号里的字面量，于是 `git rm -rf .` 这类把 `rm` 包了一层的写法从硬拦下整个漏过；分段之后 `base_cmd` 与 flag 本来就是现成的，判断放在那里才不依赖锚点
- **flag 取值和位置参数一样要过危险路径检查**。真正被动到的路径常常只出现在 `-o`、`-C`、`--prefix=` 后面，只扫位置参数等于放过落点。`safe_flags` 的取值类型因此分成三档：声明为 `number`/`string` 的不当路径扫（否则 commit message 里写个 `/etc` 就被拦），声明为 `path` 的按命令自己的类别校验，声明为 `outpath` 的（`curl -o`、`wget -O`）恒按写操作校验——这两个命令本身是 read 档，走自己的类别会被直接放行，而写入目标与重定向目标是同一件事
