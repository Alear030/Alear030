...
| `cwd` | stdio | 子进程工作目录。`${VAR}` 与其它字段同一套展开规则；未填写、`null` 或空字符串时默认仓库根，其它非字符串值（包括 `0`、`false`、`[]`、`{}`）报配置错误并跳过该 server；展开后仍是相对路径的也按仓库根解析，与从哪个目录启动无关；`args` 里的相对路径通常按这个目录解析，但最终取决于 server 程序自身 |
...
这些目录存的都是真实运行数据（会话记录、trace 与诊断日志、派生记忆、模型权重）。除 `local_model/` 下已跟踪的代码与模型元数据外，表中路径均已 gitignore——也就是说它们不在 git 历史里，删了无法恢复。删之前先想清楚，它们不是可随意重建的缓存。
