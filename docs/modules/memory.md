...
而且它关掉的不只是「入库」——`hook/hooks/after_loop/memory_pipeline/hook.py` 的判空在 `session._session_slice()` **之前**：
...
因为它会被持续改写、且长出来的维度属于个人积累，仓库跟踪的是同名 `user_info.example.json` 种子而非文件本身，首次运行自动播种、已存在则绝不覆盖——见[配置说明](../CONFIGURATION.md#画像维度的种子文件)。
