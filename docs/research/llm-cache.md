...
> 时间线：8/25 立项（#38 #39）→ 8/29 本文成稿并定位根因 → 9/6 完成修复并用真实 trace 数据验证（新 session 首轮命中率 ~14% → ~99.3%）→ 9/6 稍晚收口时发现 attachment 投递链路当时其实是断的，99.3% 全部来自另一半修复 → 修复投递后重采，首次送出 89.5%、稳态 99.5%（内容比此前多约 1752 token）。见文末[收口：主线结论与修复](#收口主线结论与修复)。
...
| 9/6（样本 3） | 修复后，但 attachment 投递未生效 | 14953 | 14848 | 105 | 99.3% |
| 9/6（样本 4，另一次独立新 session） | 修复后，但 attachment 投递未生效 | 14956 | 14848 | 108 | 99.3% |
| 9/6 23:50（样本 5） | attachment 投递已通，**内容首次送出** | 16600 | 14848 | 1752 | 89.5% |
| 9/6 23:54（样本 6，另一次独立新 session） | attachment 投递已通，**稳态** | 16600 | 16512 | 88 | 99.5% |

修复前两个样本（不同 session）都是 ~14%。样本 3、4 的 99.3% 是在 attachment 实际没送达的状态下测的（见下方更正），请求里少了约 1752 token 的内容，因此偏乐观。样本 5、6 才是内容齐全的真实形态。

**样本 5 与 6 的差别正是这套机制的关键证据**：两次请求的 `prompt_tokens` 完全相同（都是 16600），但样本 5 miss 了 1752、样本 6 只 miss 88。那 1752 就是 attachment 携带的四块 notification 内容——它**第一次**出现在这个位置时全量 miss，四分钟后的下一个 session 就整块命中了。也就是说这是一次性成本，不是每次新 session 都要重付。

另外注意样本 5 的命中数 `14848`，和样本 3、4 一模一样：那正是 `tools` schema + system prompt 那段稳定前缀，新增的 attachment 内容全部落在它之后，没有污染它。样本 6 的 miss 只剩 88，对应时间戳加上本轮用户输入——**除了真正每次都变的那部分，其余全部进入了可复用前缀**。

这也回过头解释了 `notification` 分块为什么要共用 `order` 轴并按「稳定 → 易变」排：skill(20) → timeline(30) → memory(35) → 时间戳(50)，时间戳因此被推到整个请求前缀的最末端。它一变只影响它自己，不再连累任何东西。
...
>
> 所以样本 3、4 的 99.3% **全部来自「时间戳整块挪出 system_prompt」这一件事**，与 attachment 的投递位置无关——两个 bug 的贡献不是「暂未拆分」，是其中一个压根没参与。键名修复后已重新采样，即表中的样本 5、6。
>
> 重采之前有过一个错误预判：以为 attachment 内容进入首轮后会把命中率长期拉低到 85% 上下。样本 6 证伪了它——**那部分内容自己也会被缓存**，代价只发生在它变化后的第一个 session。预判之所以错，是因为把「这次是新内容」直接当成了「每次都是新内容」。
>
