...
- 构想与实现两种语境常同时出现在一轮对话里，两者的规则不同甚至相反：
...
- 无行为影响的纯文本改动（错字、日志、用户指定的重命名、注释）可直接做，不需要单独的规划闸门。
...
- `$alear030-issue-mark`：按项目标签体系（tech-debt/boundary-violation/eval-require/atomic 等）记录审查或复盘发现的问题。想找能独立修完、不需要项目上下文积累的 issue，用 `gh issue list --repo Alear030/Alear030 --label atomic`；判据见该技能，开工后发现不满足判据时停下报告是哪一条，不要自行补做取舍。
...
- `$alear030-push-merge`：commit 之后把分支发出去，分两段。第一段 push 并开 PR，到此停下交回用户；第二段合并进 master、清理分支与 worktree、同步本地 master，只在用户明确放行后才跑。PR 本身就是评审闸门，开完不自动合并。本项目走 GitHub PR 而非本地 merge；`master` 与 `Alear030_dev` 为常驻分支，永不删除。
