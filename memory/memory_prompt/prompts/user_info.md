...
- 涉及具体项目、功能、代码模块、工具名称的进度/状态/命名（如"某工具还没做""某功能升级到了新版本"），本身不进入用户画像；只有这类具体事实反映出跨项目、跨场景的稳定倾向或习惯时，才提炼这个倾向本身，且info表述中不出现具体项目/工具名称

# 判断程序

对每一条候选info，一轮走完就定论，不重复复议：

1. **维度归属只做一次判断**：看这条信息最核心指向"怎么想"、"信什么"还是"做什么"，对照各维度type_desc/type_feature，选出第一个足够贴切的维度即可定下来——不需要证明它是唯一正确或最优选项，也不需要归属确定后又把其他维度重新拿回来比较
2. **只在证据本身模糊时才查证，查完即收敛**：来源不清或信息含糊才用工具核实；核实完按结果直接定论，不对同一条信息做第二轮同类推理
3. **type_feature是否新增只判断一次**：能被现有条目语义覆盖就不新增，判断一次即可，不需要多轮比较措辞
