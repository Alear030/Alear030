...
@register_tool(tool_name='file_read',tool_desc=tool_desc,tool_prompt=tool_prompt,tool_enabled=True,role=['main'])
def file_read(path:str,offset:int=0,limit:int=2000)->str:
    file_path = Path(path)

    if not file_path.absolute():
        return f'错误:file_path必须是绝对路径，收到{path}'
    
    if not file_path.exists():
        return f'错误:文件不存在:{path}'
    
    if file_path.is_dir():
        return f'错误:这是一个目录而非文件:{path}'
    ...
    try:
        ...
        return f'错误:文件不是UTF-8编码,无法正确读取:{path}'
    except Exception as ee:
        return f'错误:读取文件失败:{ee}'

    if not file_content.strip():
        return f"警告：文件内容为空:{path}"
