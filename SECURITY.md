...
- **写**：默认情况下 `file_write` 与 `file_edit` 的目标路径必须落在 `workspace/` 或 `skill/` 之内，否则直接拒绝；这层限制由 `config.py` 的 `FILE_EDIT_SANDBOX` 控制，关闭后可写任意绝对路径
- **读**：`file_read`、`file_grep`、`file_glob` **可以读磁盘上任意绝对路径**，只要求路径是绝对的

也就是说，沙箱开启时模型不能往工作空间外写东西，但能读到你机器上它有权限读的任何文件。如果本机有不希望被读到的内容，这一点要心里有数。
...
- **对话内容会落盘**到 `session/session_detail/`、`memory/` 与 `eval/trace/trace_log/` 下，其中 trace 还记着工具返回的原文（读过的文件、抓取的网页）；`log/log_data/` 的诊断日志在异常路径上也会带出 attachment 正文（画像、时间线、会话摘要）。这些目录已在 `.gitignore` 里，但如果你要分享仓库或截图，注意里面是真实对话
