...
The current mainstay. `CLAUDE.md` and [the skills under `.claude/skills/`](.claude/skills/README.en.md) are all for it. Cross-module mechanism changes, user-facing docs, the CHANGELOG, and commit messages basically all go through it.
...
The rules under `.cursor/rules/` are for it.
...
- `.cursor/rules/` — the alwaysApply rules for Cursor
- `.claude/skills/` — the collaboration skills for coding agents, described one by one in the [skills catalog](.claude/skills/README.en.md)
...
One is `.claude/skills/`, the skills I wrote for coding agents. The other is `skill/`, Alear030's own runtime skill system.
...
Some are produced by actively creating a skill: step on a rake once, or repeat something once, and out comes one. Thirteen skills, 1201 lines, all accumulated that way.
