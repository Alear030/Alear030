...
- **`atomic` 也是叠加标签，叠在 `tech-debt` 上**（仓库已建，绿色 `#0e8a16`，定义："方案已写明倾向、不改跨模块契约、验收可机械判定，不需要本项目上下文积累即可独立修完的问题"）。它服务的是「把能外包的 issue 一次筛出来交给其他 agent」——筛的人只跑 `gh issue list --label atomic`，不再逐条读正文，所以这个标签错打的代价比漏打高：漏打只是少派一条，错打会让一个没有上下文的执行者在需要判断的地方自己做判断。判据是「一个只读 issue 正文和证据文件的执行者，能不能改完并自证改对」，落到三条，缺一条就不打：
  ...
  打之前还要确认证据里的文件路径现在能打开——执行者只读正文和证据，路径失效等于没给证据，先在 issue 里更正再打。它与 `research`、`eval-require` 天然互斥。改动小不等于原子化：#150 只改几行注解，但正文留了一处「需拍板」的取舍，不打；#54 能照抄同文件现成的带锁 updater，但涉及并发写和真实画像数据，也不打；#133 只加一个注销方法，但它给 subagent 补的是生命周期终点、跨 agent 与 tool 两个模块，也不打。反过来，#140 要在 trace 与 log 两个模块各改一处同形的序列化，互不依赖，方案已倾向 A、验收就是比较 `splitlines()` 与 `split('\n')` 的条数，打。
  - 这个判断会随 issue 变化：取舍拍板后在 issue 里补一句结论，就可以补打。执行者开工后发现不满足，先在报告里指出不满足哪一条；摘标签、留原因是改 GitHub 状态，按执行者所在流程的闸门走（`$alear030-issue-fix` 里要经用户确认）。
- 严重度用**标题前缀**区分：`[高]` / `[中]` / `[低]`。
