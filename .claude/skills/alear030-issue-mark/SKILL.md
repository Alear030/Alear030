...
- **`atomic` 也是叠加标签，叠在 `tech-debt` 上**（仓库已建，绿色 `#0e8a16`，定义："方案已定、改动封闭在单模块、验收可机械判定，不需要本项目上下文积累即可独立修完的问题"）。它服务的是「把能外包的 issue 一次筛出来交给其他 agent」——筛的人只跑 `gh issue list --label atomic`，不再逐条读正文，所以这个标签错打的代价比漏打高：漏打只是少派一条，错打会让一个没有上下文的执行者在需要判断的地方自己做判断。判据是「一个只读 issue 正文和证据文件的执行者，能不能改完并自证改对」，落到三条，缺一条就不打：
  - **方案已定**：正文里没有「需拍板」的取舍、没有「到时候再定」的开口。列了多个方案，但验收标准只靠其中一个就能全部满足，也算已定
  - **改动封闭**：落在一个模块内，不改跨模块的契约、注册协议或生命周期，不涉及并发、时序和锁
  - **验收可机械判定**：每条验收都能用单测、探针或 grep 判定；不需要跑真实模型、读 trace、比较输出质量，也不碰 `session_detail`/`memory_storages` 这类真实数据

  所以它与 `research`、`eval-require` 天然互斥。改动小不等于原子化：#150 只改几行注解，但正文留了一处「需拍板」的取舍，不打；#54 能照抄同文件现成的带锁 updater，但涉及并发写和真实画像数据，也不打。反过来，#140 要改两个文件的序列化，方案已倾向 A、验收就是比较 `splitlines()` 与 `split('\n')` 的条数，打。
  - 这个判断会随 issue 变化：取舍拍板后在 issue 里补一句结论，就可以补打；执行者开工后发现不满足，摘掉标签并在 issue 里留一句原因，比硬着头皮做完更有用。
- 严重度用**标题前缀**区分：`[高]` / `[中]` / `[低]`。
