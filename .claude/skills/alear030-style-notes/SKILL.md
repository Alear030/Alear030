...
8. **能一行说清就不写多行** — 落笔前先问这句能不能压成一行，能就压。多行只留给一行确实装不下的因果链。**写出「因为……所以……而且……」这种论证结构，基本就是超了**——那是在向读者陈情，不是在标注。
9. **记账/日志的 `detail` 放原始值，不放散文式解释** — `{"prompt_name":..., "type":..., "target":...}` 比 `{"error":"分块声明了非法 type"}` 有用：前者让读的人自己看出哪里不对，后者是预先替他下了结论，还会随代码演进变成假话。
