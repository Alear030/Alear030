...
从 GitHub Projects 看板的 `pre-todo` 列认领一个 issue，然后：开分支 → 规划（**停下来等我确认**）→ 开发 → 验证 → 自检 → 提交 → 合并（交给 `alear030-push-merge`）→ 把看板推到 done → 问我要不要接下一个。
...
所以这个技能不装知识，装接线：靶子分类直接用[那篇复盘](../../docs/retrospective/eval-to-architecture.md)已有的四类，机制事实一律写成探针指回 `docs/`（只给查法不给答案，答案会过期，而过期的答案比没有答案更糟），技能自己最初只留三道对账、分流纪律，和一条退出标准。

**退出标准是最初版本里唯一算新增的东西**：不是「diff 读完了」就算完，是三道对账过完、每条数据流的生产者—消费者—可达性都有结论才算完。漏掉的东西通常不是看不懂，是停太早。
...
后来又补了一层：审查主体改由不带会话历史的执行者来跑，并加了一道对抗式输入审视。起因是和零上下文的外部审查器对照——假值绕过校验、参数拼路径出界、判活启发式失效、合计和表格对不上，这几类它稳定抓得到，自审稳定漏。漏的原因不是看不懂，是审查者继承了写代码时的默认假设；拿掉那段会话历史，默认才重新变回问题。
...
- `alear030-issue-fix` 与 `alear030-issue-pretodoHandle` 声明分工：前者当前 checkout 修到 commit 为止，后者管看板认领、分支与 PR；测试与提交环节分别指向 `alear030-verify`、`alear030-commit-message`，不复述
