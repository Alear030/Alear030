...
**审查走 `alear030-pr-review`，由用户开口才跑，本技能不代劳、也不自动派。** 自动审会把这个
停顿掏空：形式上还停着，实际上用户面前从「一份要自己读的 diff」变成「一份已经被消化过的结论」，
而那份结论是同一个模型家族出的。要不要审、什么时候审，是用户的决定。
...
- 告诉用户可以要一次审查（走 `alear030-pr-review`），并说明**不开口就不会审**——别让用户以为
  沉默等于已经审过了
...
- ⏸ 用户评审：自己读，或开口要一次 `alear030-pr-review`。本技能不代劳、不自动派、不催促。
- 本技能第二段：用户放行后合并并清理干净。
